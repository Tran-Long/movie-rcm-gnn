...
data = MovieLens("./data", transform=rating_transform, user_subset=0.5).get()
...
trainer = L.Trainer(accelerator="cpu", max_steps=config.max_steps, logger=logger, val_check_interval=config.val_check_interval, log_every_n_steps=10)
trainer.fit(
    lightgcn,
